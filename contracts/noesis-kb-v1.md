...
`kb_technical` is additive and returns a
`noesis-technical-research-v1` payload. Package and artifact identity is
exact: canonical ecosystem coordinates and explicitly recorded aliases are
accepted, while ambiguous names are never guessed. Graph traversal is
cycle-safe, optional dependencies are opt-in, incomplete records and
