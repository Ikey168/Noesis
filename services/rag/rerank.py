...
try:
    ...
    logger.warning(
        "sentence-transformers not available. Reranking will use fallback scoring."
    )
...
class CrossEncoderReranker:
    ...
    def __init__(
        ...
        if self.is_enabled and HAS_SENTENCE_TRANSFORMERS:
            ...
            logger.warning(
                "Reranking enabled but sentence-transformers not available. Using fallback."
            )

    def _check_reranking_enabled(self) -> bool:
        """Check if reranking is enabled via environment variable."""
        return os.getenv("ENABLE_RERANKING", "false").lower() in (
            "true",
            "1",
            "yes",
            "on",
    ...
    def _load_model(self):
        ...
        try:
            if self.model_name == "Qwen/Qwen3-Reranker-0.6B":
                ...
                self.model = QwenReranker(
                    device=self.device or "cpu", max_tokens=self.max_length
                )
            else:
    ...
    def rerank(
        ...
        try:
            ...
            for candidate in candidates:
                ...
                doc_text = (
                    f"{candidate.get('title', '')} {candidate.get('content', '')}"
                )
            ...
            for i, (candidate, rerank_score) in enumerate(
                ...
                original_score = candidate.get(
                    "score",
                    candidate.get("similarity_score", candidate.get("rank", 0.0)),
                )
    ...
    def suggest_with_jev(
        ...
        *,
        ...
        """Opt-in source-bound suggestion in the existing rerank result shape.

        Candidates must carry exact `source_ref` and `locator` fields. This
        method does not enable Jev as a default backend. It returns every
        candidate, including when hosted scoring is unavailable.
        """
        ...
        suggestion = suggest_shortlist_rerank(
            runtime,
            namespace,
            run_id,
            query,
    ...
    def _fuse_scores(
        ...
        if fusion_strategy == "weighted":
            ...
            logger.warning(
                f"Unknown fusion strategy: {fusion_strategy}. Using weighted."
            )
            return fusion_weight * original_score + (1 - fusion_weight) * rerank_score

    def _fallback_rerank(
        self, candidates: List[Dict[str, Any]], top_k: Optional[int] = None
    ) -> List[RerankResult]:
        """Fallback reranking that preserves original order."""
        results = []
        for i, candidate in enumerate(candidates):
            original_score = candidate.get(
                "score", candidate.get("similarity_score", candidate.get("rank", 0.0))
            )
