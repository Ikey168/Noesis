...
An opt-in real-model stress check (`NOESIS_LIVE_NLI=1 pytest tests/integration/kb/test_nli_long_live.py`) covered all 9,779 source characters across three windows and rejected the oversized direct pair. Its recorded predictions are in `workflow-implementation-evidence/nli-long-evidence.json`; the generated stress input establishes window mechanics, not human-verified entailment. In particular, the model also assigned entailment to background-only windows, reinforcing why its raw predictions are not a factual-support certificate.
