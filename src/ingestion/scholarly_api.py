...
def provider(source):
    ...
    for name, host in [('crossref', 'api.crossref.org'), ('openalex', 'api.openalex.org')]:
        if source['source_id'] == name + '-works' and endpoint.hostname == host and endpoint.path.rstrip('/') == '/works':
            return name
    return None
...
def parameters(name, request, *, cursor, limit, contact=None, secret=None):
    ...
    mapping = {'doi': 'doi', 'author': 'query.author' if name == 'crossref' else 'authorships.author.id',
               'venue': 'query.container-title' if name == 'crossref' else 'primary_location.source.id',
               'concept': 'concepts.id'}
    for key, target in mapping.items():
        ...
        if value is not None:
            if name == 'crossref' and target.startswith('query.'):
                ...
                filters.append(target + ':' + str(value).removeprefix('https://doi.org/'))
    for key, native in [('from_ms', 'from'), ('to_ms', 'until' if name == 'crossref' else 'to')]:
        if request.get(key) is not None:
            date = datetime.fromtimestamp(int(request[key]) / 1000, timezone.utc).date().isoformat()
            filters.append(native + ('-pub-date:' if name == 'crossref' else '_publication_date:') + date)
    ...
    result['rows' if name == 'crossref' else 'per_page'] = min(limit, 1000 if name == 'crossref' else 200)
...
def records(name, payload, *, cursor, limit):
    envelope = payload.get('message') if name == 'crossref' and isinstance(payload, dict) else payload
    ...
    output = []
    for item in values:
        if not isinstance(item, dict) or not item.get('DOI' if name == 'crossref' else 'id'):
            raise ValueError('native scholarly record lacks stable identifier')
        doi = str(item.get('DOI') or item.get('doi') or '').removeprefix('https://doi.org/').lower()
        if name == 'crossref':
            title = item.get('title') or []
            if not isinstance(title, list):
                raise ValueError('Crossref title must be an array')
            title = str(title[0]) if title else ''
            parser = _Text(); parser.feed(item.get('abstract') or ''); parser.close()
            abstract = ''.join(parser.parts).strip()
            authors = [' '.join(str(a.get(k) or '') for k in ('given', 'family')).strip() or a.get('name', '') for a in item.get('author', [])]
            parts = (item.get('published') or item.get('issued') or {}).get('date-parts', [[]])[0]
            date = '-'.join(str(v).zfill(4 if i == 0 else 2) for i, v in enumerate(parts)) or None
        ...
        output.append({**item, 'id': identity, 'doi': doi or None, 'url': url, 'title': title,
                       'authors': authors, 'published_at': date, 'content': abstract or title,
                       'abstract_available': bool(abstract),
                       'content_representation': 'plain-text-abstract' if abstract else 'title-only'})
    next_cursor = envelope.get('next-cursor') if name == 'crossref' else (envelope.get('meta') or {}).get('next_cursor')
